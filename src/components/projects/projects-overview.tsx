...
export function ProjectsOverview() {
  ...
  const exportSummary = {
    ...
    trend: trendData,
    trendComparison,
    projects: selectedReport ? [selectedReport] : filteredReports,
  ...
  return (
    <section className="space-y-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-6">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 text-4xl font-semibold tracking-tight text-white">
              Projetos com contraste forte, risco visivel e leitura de telao.
            </h2>
        ...
        <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_220px]">
          <label className="relative block">
            ...
            <input
              ...
              className="min-h-12 w-full rounded-2xl border border-slate-300 bg-white pl-9 pr-3 text-sm text-slate-900 focus:border-[#ee2331]"
            />
          </label>
          <select
            value={activeFilter}
            onChange={(event) => setActiveFilter(event.target.value as ActiveFilter)}
            className="min-h-12 rounded-2xl border border-slate-300 bg-white px-3 text-sm text-slate-900"
          >
            <option value="todos">Todos os status</option>
            <option value="ativos">Ativos</option>
            <option value="inativos">Inativos</option>
          </select>
          <div className="grid grid-cols-2 rounded-2xl border border-slate-300 bg-slate-950 p-1 text-sm">
            <button
              type="button"
              onClick={() => setViewMode("lista")}
              className={`inline-flex items-center justify-center gap-2 rounded-2xl px-3 py-2 font-medium ${
                viewMode === "lista"
      ...
      {isLoading ? <LoadingPanel message="Carregando projetos e relatórios..." /> : null}
      {error ? <ErrorPanel message={error} /> : null}

      {!isLoading && !error && filteredReports.length === 0 ? (
        <div className="rounded-[28px] border border-slate-200 bg-white p-5 text-sm text-slate-600 shadow-lg">
          Nenhum projeto encontrado para os filtros selecionados.
        </div>
      ) : null}

      {!isLoading && !error && filteredReports.length > 0 && viewMode === "lista" ? (
        <div className="overflow-x-auto rounded-[28px] border border-slate-200 bg-white shadow-xl">
